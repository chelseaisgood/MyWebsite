...
$(function(){
	...
	function handleReturn(call) {
		...
		case 'invalid_username_length':
			indexShowTooltip('reg_user','Length of username should be 6-16  and only accept a-z, A-Z, 0-9, _ ');
			break;
		...
		case 'repwd_not_equal':
			...
			indexShowTooltip('reg_repwd','Two different password entered');
			break;
		case 'invalid_pwd_length':
			$("#reg_pwd").val('');
			$("#reg_repwd").val('');
			//alert("pwd length should between 8-16");
			indexShowTooltip('reg_repwd','Password length should between 8-16');
			break;
		case 'invalid_pwd_character':
			$("#reg_pwd").val('');
			$("#reg_repwd").val('');
			//alert("invalid pwd format");
			indexShowTooltip('reg_repwd','Invalid password format');
			break;
